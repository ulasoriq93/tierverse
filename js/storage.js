...
function normalizeList(input={}){
 ...
 return {id:listId,title:cleanText(input.title,160)||'Untitled ranking',description:String(input.description??'').slice(0,2000),category:APP.categories.includes(input.category)?input.category:'Other',tags:(Array.isArray(input.tags)?input.tags:[]).map(t=>cleanText(t,32)).filter(Boolean).slice(0,8),favorite:!!input.favorite,accent:safeColor(input.accent||'#8b5cf6'),cover:safeImageSrc(input.cover)||null,coverMode:['fit','fill'].includes(input.coverMode)?input.coverMode:'fit',lockTierOrder:input.lockTierOrder!==false,createdAt:input.createdAt||now(),updatedAt:input.updatedAt||now(),tiers,items};
}
